...
class DirectedBrownianMotion:
    ...
    def __init__(self,
        ...
        self.do_discard_trajectories_beyond_boundaries = True
        ...
        self.rewards = torch.zeros(self.num_trajectories).to(device=self.device)

        if self.do_saturate or self.do_discard_trajectories_beyond_boundaries:
        ...
        # if self.do_discard_trajectories_beyond_boundaries:
        self.rewards_subset = None
        self.action_samples_ii = None
        ...
        # Elite actions:
        self.elites_actions = torch.zeros((self.num_elites,self.planning_horizon,self.dim_action)).to(device=self.device)
        self.mean_actions_optimized_elites = torch.zeros((self.planning_horizon,self.dim_action)).to(device=self.device)
        self.actions_optimized_elites_var = torch.zeros((self.planning_horizon,self.dim_action)).to(device=self.device)
    ...
    def optimize(self, function, action_samples_init, std_action_samples, minimize=True):
        ...
        self.mean_actions_optimized_elites[:] = action_samples_init
        ...
        while ii < self.max_iters:
            ...
            self.n_samples[:] = 0.0
            self.action_samples_ii[:,0,:] = self.mean_actions_optimized_elites[0,:].repeat((self.num_trajectories,1)) # Start with the first action of the warm start, repeated across all trajectories
            for t in range(self.planning_horizon-1):
                ...
                while self.ind_rejected.sum() > 0:
                    ...
                    # Pass it through the correlation matrix:
                    if self.use_correlation_matrix:
                        sample_subset[:] = sample_subset.matmul(self.corr_matrix_chol)
            ...
            #     self.action_samples_ii_subset = self.action_samples_ii
            #     self.rewards_subset = self.rewards

            if self.do_discard_trajectories_beyond_boundaries:
                # import pdb; pdb.set_trace()
                ind_traj_plan_below_upper = (((self.action_samples_ii < self.upper_bound_all).sum(2)).sum(1) == self.planning_horizon*self.dim_action)
                ind_traj_plan_above_lower = (((self.action_samples_ii > self.lower_bound_all).sum(2)).sum(1) == self.planning_horizon*self.dim_action)
                ind_valid = ind_traj_plan_below_upper & ind_traj_plan_above_lower
                # import pdb; pdb.set_trace()
                if ind_valid.any():
                    self.action_samples_ii_subset = self.action_samples_ii[ind_valid,:,:]
                    print("Some of the trajectiries ARE WITHIN LIMITS")
                else:
                    print("NONE of the trajectiries ARE WITHIN bounaries")
                ...
                self.rewards_subset = function(self.action_samples_ii_subset)
                # TODO: Watch out that in this case, self.rewards is not updated.
                # If someone tries to access self.rewards, will find a tensor of zeroes
            

            ii += 1

            # Get elite rewards:
            best_optimized_rewards, ind_best_optimized_rewards = torch.topk(self.rewards_subset, k=self.num_elites, dim=0, largest=not minimize) # TODO alonrot: get largest element from values | ~0.06ms for 100 trajectories, and 10 elites
            self.elites_actions[:] = self.action_samples_ii_subset.index_select(dim=0, index=ind_best_optimized_rewards)

            self.mean_actions_optimized_elites[:] = torch.mean(self.elites_actions, dim=0)
            self.actions_optimized_elites_var[:] = torch.var(self.elites_actions, unbiased=False, dim=0)



            # RAD2DEG = 180./np.pi
            # action_samples_np = self.action_samples_ii_subset.cpu().numpy()
            # actions_optimized_elites_mean_np = self.mean_actions_optimized_elites.cpu().numpy()
            # # import pdb; pdb.set_trace()
            ...
            #     if kkk == 0:
            #         hdl_plot.plot(self.elites_actions[kkk,:,ind1].cpu().numpy()*RAD2DEG,self.elites_actions[kkk,:,ind2].cpu().numpy()*RAD2DEG,color="mediumpurple",label="Elites",marker=".",linewidth=1.0)
            #     else:
            #         hdl_plot.plot(self.elites_actions[kkk,:,ind1].cpu().numpy()*RAD2DEG,self.elites_actions[kkk,:,ind2].cpu().numpy()*RAD2DEG,color="mediumpurple",marker=".",linewidth=1.0)
        ...
        if self.do_saturate:
            self.error_checking_action_solutions_within_bounds(self.mean_actions_optimized_elites)

        print("@Optimizer PDDM: finished! ii = {0:d} / {1:d}".format(ii,self.max_iters))

        return dict(mean_actions_optimized_elites=self.mean_actions_optimized_elites, 
                    actions_optimized_elites_var=self.actions_optimized_elites_var, 
                    ...
                    saturations_count=self.saturations_count) # Keep it for compatibility with PDDM
...
class DirectedBrownianMotionOptimizer(Optimizer):
    ...
    def __init__(self,
        ...
        self.dir_bro_mo = DirectedBrownianMotion(self.planning_horizon,
                        self.dim_action,
                       self.lower_bound,
                       self.upper_bound,
                       num_trajectories,
        ...
        self.action_sequence2return.all_returns_with_particles = None
    ...
    def reset_trajectory(self):
        ...
        # var = torch.pow(self.lower_bound - self.upper_bound, 2) / 256.
        var = torch.pow(self.lower_bound - self.upper_bound, 2) / 80.
    ...
    def plan_action_sequence(self, state0) -> ActionSequence:
        ...
        def func(actions):
            ...
            if self.trajectories_last_first_time:
                ...
                self.action_sequence2return.all_returns_with_particles = out.all_returns_with_particles
            else:
                # self.trajectories_last[:,:,:,:] = out.all_trajectories
                # self.action_sequence2return.all_trajectories[:] = out.all_trajectories
                self.action_sequence2return.all_trajectories = out.all_trajectories # We chaged this because in the brownian motion directed we are have the flag self.do_discard_trajectories_beyond_boundaries = True
                # self.action_sequence2return.all_returns[:] = out.all_returns
                self.action_sequence2return.all_returns = out.all_returns
        ...
        solution = self.dir_bro_mo.optimize(func, self.mean_actions_optimized_elites, self.std_action_samples, minimize=False)
        ...
        self.mean_actions_optimized_elites[0:-1,:] = self.mean_actions_optimized_elites[1::,:].clone() # clone() is important here | Move up all the actions (i.e., consume the first actions)
        ...
        # Do here the curiosity based thing:
        # self.action_sequence2return.all_returns_with_particles
    ...
    def __str__(self):
        return F"PDDMOptimizer (horizon={self.planning_horizon}, " \
            ...
            F"max iter={self.dir_bro_mo.max_iters})"
